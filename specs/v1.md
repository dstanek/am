...
split-pane layout, and optional containerization via Podman or Docker.
...
user = "am"            # username inside the container image
...
- `WorktreeError(String)` — git or jj CLI error
...
pub struct ContainerConfig {
    ...
    pub user: String,               // username inside the container, default "am"
...
| `<repo-root>/.git` _(colocated jj+git only)_ | same path | read-write |
| `.am/gitconfig` (or override) | `/home/<user>/.gitconfig` | read-only |
| `~/.ssh` | `/home/<user>/.ssh` | read-only |

`<user>` is `container.user` from config (default `"am"`).

For colocated jj+git repos both `.jj` and `.git` are mounted at their host paths.
...
| Preset key | Host path | Container path | Mode | Notes |
|---|---|---|---|---|
| `claude` | `~/.claude` (or `$CLAUDE_CONFIG_DIR`) | `/home/<user>/.claude` | read-write | Claude Code session + credentials |
| `claude` | `~/.claude.json` | `/home/<user>/.claude.json` | read-write | Claude account settings |
| `copilot` | `~/.config/gh` | `/home/<user>/.config/gh` | read-only | GitHub CLI auth token |
| `copilot` | `~/.config/github-copilot` | `/home/<user>/.config/github-copilot` | read-only | GitHub Copilot CLI |
| `gemini` | `~/.gemini` | `/home/<user>/.gemini` | read-only | Gemini CLI |
| `codex` | _(none — env var only)_ | — | — | `OPENAI_API_KEY` injected via `-e` |

`<user>` is `container.user` from config (default `"am"`).

If `container.agent` is set to an unrecognised value, `am` errors early with a list of
valid agent names — it does not silently proceed.

```rust
pub fn preflight_agent_auth(agent: KnownAgent, container_user: &str) -> anyhow::Result<AgentAuth>
```
...
pub fn build_run_command(
    ...
    extra_env: &[(String, String)],  // agent-injected env vars (e.g. OPENAI_API_KEY for codex)
    network: NetworkMode,
    working_dir: &str,
    container_name: &str,
) -> Vec<String>
...
Produces a command of the form (git repo, Claude preset, Linux/Podman):

```
podman run --rm -it \
  --name am-<slug> \
  -v /home/user/repo/.am/worktrees/feat:/home/user/repo/.am/worktrees/feat:rw,z \
  -v /home/user/repo/.git:/home/user/repo/.git:rw,z \
  -v /home/user/repo/.am/gitconfig:/home/am/.gitconfig:ro,z \
  -v /home/user/.ssh:/home/am/.ssh:ro,z \
  -v /home/user/.claude:/home/am/.claude:rw,z \
  -v /home/user/.claude.json:/home/am/.claude.json:rw,z \
  -e ANTHROPIC_API_KEY \
  --workdir /home/user/repo/.am/worktrees/feat \
  <image>
```
...
### `am start <slug> [--agent <cmd>] [--no-container]`
...
6. If `is_in_tmux()`:
   ...
   g. Switch tmux focus to `am-<slug>` window
...
5. **Container startup delay** — originally proposed as a configurable `startup_delay_ms`.
   Removed in implementation: the agent is instead appended directly to the container CMD
   (e.g. `podman run … my-image claude`), so no delay is needed.
